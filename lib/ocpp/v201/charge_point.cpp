...
ChargePoint::ChargePoint(const std::map<int32_t, int32_t>& evse_connector_structure,
    ...
    this->database_connection = std::make_shared<common::DatabaseConnection>(fs::path(core_database_path) / "cp.db");
    this->database_handler = std::make_shared<DatabaseHandler>(*this->database_connection, fs::path(sql_init_path));
