...
bool Callbacks::all_callbacks_valid(std::shared_ptr<DeviceModel> device_model) const {
    ...
    if (valid) {
        ...
        if (device_model->get_optional_value<bool>(ControllerComponentVariables::V2XChargingCtrlrEnabled)
                .value_or(false) and
            device_model->get_optional_value<bool>(ControllerComponentVariables::ISO15118CtrlrEnabled)
                .value_or(false)) {
            if (!this->update_allowed_energy_transfer_modes_callback.has_value() or
                ...
                EVLOG_error << "V2XCharging and ISO15118 are both marked as 'Enabled', but "
