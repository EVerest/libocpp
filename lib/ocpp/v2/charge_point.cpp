...
void ChargePoint::initialize(const std::map<int32_t, int32_t>& evse_connector_structure,
    ...
    this->data_transfer = std::make_unique<DataTransfer>(*this->block_context, this->callbacks.data_transfer_callback,
                                                         DEFAULT_WAIT_FOR_FUTURE_TIMEOUT);
    this->security = std::make_unique<Security>(*this->block_context, *this->logging, this->ocsp_updater,
    ...
    if (device_model->get_optional_value<bool>(ControllerComponentVariables::ReservationCtrlrAvailable)
        ...
        this->reservation = std::make_unique<Reservation>(
            *this->block_context, this->callbacks.reserve_now_callback.value(),
            this->callbacks.cancel_reservation_callback.value(), this->callbacks.is_reservation_for_token_callback);
    }

    this->authorization = std::make_unique<Authorization>(*this->block_context);
    this->authorization->start_auth_cache_cleanup_thread();

    this->diagnostics = std::make_unique<Diagnostics>(
        *this->block_context, *this->authorization, this->callbacks.get_log_request_callback,
    ...
    if (device_model->get_optional_value<bool>(ControllerComponentVariables::DisplayMessageCtrlrAvailable)
        ...
        this->display_message = std::make_unique<DisplayMessageBlock>(
            *this->block_context, this->callbacks.get_display_message_callback.value(),
            this->callbacks.set_display_message_callback.value(),
            this->callbacks.clear_display_message_callback.value());
    }

    this->meter_values = std::make_unique<MeterValues>(*this->block_context);

    this->availability = std::make_unique<Availability>(*block_context, this->callbacks.time_sync_callback,
    ...
    this->smart_charging =
        std::make_unique<SmartCharging>(*this->block_context, this->callbacks.set_charging_profiles_callback);

    this->tariff_and_cost = std::make_unique<TariffAndCost>(
        *block_context, *this->meter_values, this->callbacks.set_display_message_callback,
        this->callbacks.set_running_cost_callback, this->io_service);

    this->firmware_update = std::make_unique<FirmwareUpdate>(*this->block_context, *this->availability, *this->security,
                                                             this->callbacks.update_firmware_request_callback,
                                                             this->callbacks.all_connectors_unavailable_callback);

    this->transaction = std::make_unique<TransactionBlock>(
        *this->block_context, *this->message_queue, *this->authorization, *this->availability, *this->smart_charging,
    ...
    this->provisioning = std::make_unique<Provisioning>(
        *this->block_context, *this->message_queue, this->ocsp_updater, *this->availability, *this->meter_values,
        *this->security, *this->diagnostics, *this->transaction, this->callbacks.time_sync_callback,
        this->callbacks.boot_notification_callback, this->callbacks.validate_network_profile_callback,
        this->callbacks.is_reset_allowed_callback, this->callbacks.reset_callback,
        this->callbacks.stop_transaction_callback, this->callbacks.variable_changed_callback,
        this->registration_status);

    this->remote_transaction_control = std::make_unique<RemoteTransactionControl>(
        *this->block_context, *this->transaction, *this->smart_charging, *this->meter_values, *this->availability,
