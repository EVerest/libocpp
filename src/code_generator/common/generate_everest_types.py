...
def get_data_type_dependencies(data_type: DataType) -> List[str]:
    ...
    """
    dependencies = [property.ref for property in data_type.properties if property.ref]
    for property in data_type.properties:
        if property.ref:
            dependencies.append(property.ref)
        elif property.item and property.item.ref:
            dependencies.append(property.item.ref)
    return dependencies
    ...
    for data_type in data_types:
        ...
        for dep_type in depends_on:
            for i in range(len(sorted_types)):
                ...
                if sorted_types[i].name == dep_type:
...
def parse_item(property: Dict):
    """
    Parses ArrayItem of the given property
    """
    property_item_type = property.get("items").get("type", "object")
    property_item_ref = None
    if "$ref" in property.get("items"):
        if property.get("items").get("$ref").endswith("EnumType"):
            property_item_type = "string"
        property_item_ref = (
            property.get("items").get("$ref").split("/")[-1].replace("Type", "")
        )
    min_items = property.get("minItems")
    max_items = property.get("maxItems")
    return ArrayItem(property_item_type, property_item_ref, min_items, max_items)


def parse_property(
    property: Dict, property_name: str, definition_type: Dict, definitions: List[Dict]
):
    """
    ...
    property_ref = None
    if "$ref" in property:
        property_ref = property["$ref"].split("/")[-1].replace("Type", "")
    ...
    if property_type == "array":
        property_item = parse_item(property)
...
def parse_type(definition_type: Dict, definitions) -> DataType:
    ...
    """
    properties = []
    for property_name, property in definition_type.get("properties").items():
        properties.append(
            parse_property(property, property_name, definition_type, definitions)
        )
        ...
        with open(schema_file, "r", encoding="utf-8-sig") as schema_dump:
            ...
            for definition_name, definition_type in definitions.items():
                if "enum" in definition_type and definition_name in types_to_generate:
                    ...
                    type = parse_type(definition_type, definitions)
